...
TELEFONO=@telefono and TURNO=@turno)>0
	...
	begin tran
	 insert into EMPLEADOS values(@cveemp,@NombreE,@ApellidoP,@ApellidoM,@telefono,@turno)
	commit tran
...
set @Ban=3
	...
	set @Ban=6
		...
		ELSE
			...
			begin tran
			insert into PROVEDORES values(@cveprov,@NombPro,@TelPro,@dirección)
			commit tran
		end
...
create alter procedure PAInserproductos
@cveproducto int,
